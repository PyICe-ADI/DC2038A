...
Mailbox<SMBALERT_PIN_INBOX_SIZE, SMBALERT_PIN_OUTBOX_SIZE>  smbalert_pin_mailbox    = {.my_id=SMBALERT_PIN_ADDRESS, .to_id=0, .from_id=0, .inbox_msg_size=0, .outbox_msg_size=0, .inbox_status=PACKET_ABSENT, .outbox_status=PACKET_ABSENT, .inbox_overflow=false                         };
/****************************************************************************
 *                                                                          *
 ****************************************************************************/
    ...
    ********************************/
    if(labcomm_packet.available)
        switch(labcomm_packet.destination_id)
        {
            ...
            case SMBALERT_PIN_ADDRESS:  smbalert_pin_mailbox.get_packet(&labcomm_packet);   break;
        }
   /********************************
    * Outgoing Mail                *
    ********************************/
    ...
    if(smbalert_pin_mailbox.outbox_status == PACKET_PRESENT)    smbalert_pin_mailbox.send_packet();
}